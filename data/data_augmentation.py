...
def augment_mmwave(points):
    ...
    def random_jitter(points, sigma=0.01, clip=0.05):
        ...
        jitter = torch.clamp(sigma * torch.randn_like(points[:, :, :, :3]), -clip, clip)
        points[:, :, :, :3] += jitter
        return points
...
def augment_wifi_csi(data):
    ...
    # # Normalization
    # data = (data - data.mean(dim=(1, 2, 3, 4), keepdim=True)) / (data.std(dim=(1, 2, 3, 4), keepdim=True) + 1e-6)
    
    # return data

def augment_data(data, mod):
    if mod == 'mmwave' or mod == 'lidar':
