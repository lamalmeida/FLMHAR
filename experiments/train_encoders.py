...
def train_step(mod, model, optimizer, data, device, n_clusters):
    ...
    loss = cluster_loss(proj_1, proj_2, device, n_clusters)
    loss.backward()
    optimizer.step()
    return loss.item()
...
def train_contrastive(encoder, optimizer, train_loader, val_loader, epochs, args):
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            total_loss.append(train_step(args.modalities, encoder, optimizer, data['input_' + args.modalities], args.device, args.num_classes))
        ...
        for val_data in val_loader:
            ...
            val_loss.append(validation_step(args.modalities, encoder, val_data['input_' + args.modalities], args.device, args.num_classes))
           
        print(f'[{args.modalities:<10}] Epoch {epoch + 1:>2}, Validation Loss: {np.mean(val_loss):.4f} ({time.time() - start_time:.4f})')
        torch.save(encoder.state_dict(), f'ckpt/encoders/intermediate/{args.modalities}_epoch_{epoch + 1}_valloss_{np.mean(val_loss):.2f}.pth')
    ...
    torch.save(encoder.state_dict(), f'ckpt/encoders/{args.modalities}_epoch_{epoch + 1}_valloss_{np.mean(val_loss):.2f}.pth')
