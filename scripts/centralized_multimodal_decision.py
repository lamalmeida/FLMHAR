...
from experiments.evaluation import evaluate_multimodal_decision
...
if __name__ == '__main__':
    ...
    for mod in all_mods:
        ...
        classification_heads[mod] = ClassificationHead(args.d_model, args.num_classes).to(args.device)

        params.extend(list(classification_heads[mod].parameters()))
    
    #classification_final = ClassificationHead(args.num_classes * len(all_mods), args.num_classes).to(args.device)
    ...
    for mod in all_mods:
        ...
        optimizer = torch.optim.Adam(classification_heads[mod].parameters(), lr=args.classification_learning_rate)
        train_classification(encoders[mod], classification_heads[mod], optimizer, train_loader[mod], val_loader[mod], epochs=args.classification_epochs, args=args)


    optimizer = torch.optim.Adam(params, lr=args.classification_learning_rate)

    train_decision_fusion(encoders, classification_heads, classification_final, optimizer, train_loader, val_loader, args)

    evaluate_multimodal_decision(encoders, classification_heads, classification_final, test_loader)
